...
def get_users(limit: int = Query(10, ge=1, le=100), offset: int = Query(0, ge=0), search: str = Query("", min_length=0), db: Session = Depends(get_db_session), current_admin: Admin = Depends(get_current_admin)):
    return AdminService.list_users(db, limit=limit, offset=offset, search=search)

@router.get("/posts", response_model=list[PostResponse])
def get_posts(limit: int = Query(10, ge=1, le=100), offset: int = Query(0, ge=0), search: str = Query("", min_length=0), db: Session = Depends(get_db_session), current_admin: Admin = Depends(get_current_admin)):
    return AdminService.list_posts(db, limit=limit, offset=offset, search=search)
